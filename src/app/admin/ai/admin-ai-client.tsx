...
export function AdminAiClient() {
  ...
  async function createProvider(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(response.ok ? "AI provider created." : "Create AI provider failed.");
    if (response.ok) {
  ...
  async function updateProvider(event: FormEvent<HTMLFormElement>, provider: ProviderRow) {
    ...
    const response = await fetch(`/api/admin/ai/${provider.id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        name: form.get("name"),
    ...
    setMessage(response.ok ? "AI provider updated." : "Update AI provider failed.");
    await loadProviders();
  }
  ...
  async function disableProvider(provider: ProviderRow) {
    const response = await fetch(`/api/admin/ai/${provider.id}`, { method: "DELETE" });
    setMessage(response.ok ? "AI provider disabled." : "Disable AI provider failed.");
    await loadProviders();
  }
    ...
    <div className="admin-users">
      <section className="panel admin-command">
        <form className="form compact-form" onSubmit={createProvider}>
          <h2>Create AI provider</h2>
          <div className="admin-form-grid">
            <label>
              Key
              <input name="key" required minLength={2} placeholder="gemini" />
            </label>
            <label>
              Name
              <input name="name" required minLength={2} placeholder="Gemini Flash" />
            </label>
            <label className="admin-wide">
              Config JSON
              <textarea name="config" defaultValue={'{"model":"gemini-2.0-flash","priority":1}'} />
            </label>
      ...
      <section className="admin-user-list" aria-label="AI providers">
        {providers.length === 0 && !loading ? <div className="panel muted">No AI providers found.</div> : null}
