...
try {
  for (const width of widths) {
    ...
    const result = await page.evaluate(() => ({
      ...
      bottomNav: getComputedStyle(document.querySelector(".bottom-nav") as Element).display,
      inputFontSize: getComputedStyle(document.querySelector("input") as Element).fontSize
    }));
    await page.close();
