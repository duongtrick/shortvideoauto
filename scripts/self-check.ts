...
import { csvHeaders, formatCsv } from "../src/services/csv";
...
import { addSubscriptionDays, getPlanDurationDays } from "../src/services/subscriptions";
...
assert.equal(formatCsv([{ name: "A, B", note: "He said \"ok\"" }], ["name", "note"]), "name,note\n\"A, B\",\"He said \"\"ok\"\"\"\n");
assert.equal(csvHeaders("users.csv")["content-type"], "text/csv; charset=utf-8");
...
assert.equal(getPlanDurationDays("bank_transfer:creator_monthly:30"), 30);
assert.equal(addSubscriptionDays(new Date("2026-01-01T00:00:00.000Z"), 30).toISOString(), "2026-01-31T00:00:00.000Z");
