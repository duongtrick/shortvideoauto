...
- Auth forms now parse API responses defensively and user-flow smoke tests cover register, forgot password, resend verification, reset password, login failure, and anonymous dashboard redirect without runtime crash.
...
- Pending bank payment notification foundation exists with in-app alert, email delivery, and admin-editable `billing.payment_pending` template.
- Bank-poll auto confirmation now activates pending subscription plans with the same shared helper as manual admin confirmation.
...
- Admin CSV export foundation exists for users, payments, and audit logs, with filtered UI export links.
...
- Admin modules are CRUD/foundation level, not full enterprise scope from plan:
  - CSV export is foundation-level only: users, payments, audit logs read-only filtered exports exist
  - no bulk actions
  - no advanced sorting
