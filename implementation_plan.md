...
- [x] Billing foundation: user bank transfer payment create/list, QR payload text, auto confirm via bank poll
  - [x] Foundation: pricing supports credit packs and subscription-style plans from admin-editable `pricing.config`
...
- [ ] Admin layout: sidebar, breadcrumbs, role guard
  ...
  - [x] Hardened: admin API routes return `401` for unauthenticated and `403` for non-admin using DB role guard
...
- [ ] Module 8: AI provider management (CRUD + test)
  - [x] API + UI foundation: list/create/edit config/toggle active/disable/test AI providers
